...
def tmnt_controller():
	...
	while not rospy.is_shutdown():
		a = distancefromwall-y_l
		b = distancefromwall-y_r  
		if(b >a):
			delta = a 
		else :
			delta =b	
		#PID_output = prev_PID_output + kp*delta + ki*prev_error + kd*(delta-prev_error)
		...
		linear_vel   = np.clip((s_d-0.4),-0.1,0.4)

		if ((linear_vel <0.001) and (linear_vel >-0.001)):
			angular_zvel = 0.5 
