...
def main():
    ...
    while not rospy.is_shutdown():
        if flag: 
            rrt = RRT_2D.rrt_2D(cv2.cvtColor(autonomous_rrt.map_img(autonomous_rrt.current_map), cv2.COLOR_GRAY2BGR)[::-1], start, goal)
            rrt.RRT()
            flag = False
        autonomous_rrt.points_publisher(autonomous_rrt.convertPath(rrt.path, [-192,-192],0))
        cv2.imshow('Constructed Map', autonomous_rrt.map_img(autonomous_rrt.current_map))
